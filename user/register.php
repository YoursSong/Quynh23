...
<form action="" method="POST" enctype="multipart/form-data">
              ...
              <div class="row">
            <div class="col">
                    <input type="text" class="inp-r" placeholder="Enter Name Account" name="txtname"/>
                  </div>
                </div>
                <div class="row">
            <div class="col">
                    <input type="email" class="inp-r" placeholder="Enter Email" name="txtemail"/>
                  </div>
                </div>
                <div class="row">
            <div class="col">
                    <input type="password" class="inp-r" placeholder="Enter Password" name="txtpass"/>
                  </div>
                </div>
                <div class="row">
            <div class="col">
                    <input type="password" class="inp-r" placeholder="Enter RePassword" name="txtre_pass"/>
                  </div>
                </div>
                <div>
            ...
            <?php
                ...
                if(isset($_POST['txtsub'])) {
                    $se_user = $get_data->select_user($_POST['txtname']);
                    if(empty($_POST['txtname']) || empty($_POST['txtpass']) || empty($_POST['txtemail'])) {
                        echo "<script> alert('Bạn chưa nhập đủ dữ liệu')</script>";
                    } else {
                        if(mysqli_num_rows($se_user) >= 1) {
                            echo "<script> alert('User đã có người dùng')</script>";
                        } else {
                            $se_email = $get_data->select_register_email($_POST['txtemail']);
                            if(mysqli_num_rows($se_email) >= 1) {
                                ...
                                if($_POST['txtpass'] != $_POST['txtre_pass']) {
                                    ...
                                    if (!preg_match('/^[a-zA-Z0-9_]+$/', $_POST['txtname'])) {
                                        ...
                                        if (!filter_var($_POST['txtemail'], FILTER_VALIDATE_EMAIL)) {
                                            ...
                                            $re_user = $get_data->register($_POST['txtname'], $_POST['txtemail'], $_POST['txtpass']);
                                            if($re_user) {
